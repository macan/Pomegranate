 ...
 *
 * Armed with EMACS.
 * Time-stamp: <2010-05-15 13:45:21 macan>
 *
...
void *spool_main(void *arg)
{
    ...
    while (!hro.spool_thread_stop) {
        ...
        if (err == EINTR)
            continue;
        hvfs_debug(root, "Service thread %d wakeup to handle the requests.\n",
                   sta->tid);
        ...
        while (1) {
            ...
            if (err == -EHSTOP)
                ...
                hvfs_err(root, "Service thread handle request w/ error %d\n",
                         err);
            }
        }
    }
...
int root_spool_create(void)
{
    ...
    if (!hro.spool_thread) {
        hvfs_err(root, "xzalloc() pthread_t failed\n");
        return -ENOMEM;
    }

    sta = xzalloc(hro.conf.service_threads * sizeof(struct spool_thread_arg));
    if (!sta) {
        hvfs_err(root, "xzalloc() struct spool_thread_arg failed\n");
        err = -ENOMEM;
        goto out_free;
    }
