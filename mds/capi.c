               ...
               struct xnet_msg *msg)
{
    ...
    } else if (unlikely(ai->column > HVFS_KV_MAX_COLUMN)) {
    ...
    *nr = 1;
    iov[0].iov_base = xmalloc(sizeof(u64));
    if (unlikely(!iov[0].iov_base)) {
        hvfs_err(mds, "xzalloc itbid failed\n");
        err = -ENOMEM;
        goto out;
    }
    iov[0].iov_len = sizeof(u64);
    *(u64 *)iov[0].iov_base = hi.itbid;
               ...
               struct xnet_msg *msg)
{
    ...
    if (!ai->column) {
        /* fall through quickly */;
    } else if (ai->column > HVFS_KV_MAX_COLUMN) {
...
void xtable_handle_req(struct xnet_msg *msg)
{
    ...
    /* sanity checking */
    if (unlikely(msg->tx.len < sizeof(*ai))) {
