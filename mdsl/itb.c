 ...
 *
 * Armed with EMACS.
 * Time-stamp: <2011-06-27 22:29:21 macan>
 *
...
struct storage_result *mdsl_read_itb(struct storage_index *si)
{
    ...
    path = sprintf("%s/%ld/md", hmo.conf.mdsl_home, si->sic.uuid);
    ...
    path = sprintf("%s/%ld/range.[%016d]", hmo.conf.mdsl_home, 
                   si->sic.uuid, max);
    ...
    /* get the ITB */
    path = sprintf("%s/%ld/itb", hmo.conf.mdsl_home, si->sic.uuid);
    sr = get_free_sr();
    sr->flag = SR_ITB;
